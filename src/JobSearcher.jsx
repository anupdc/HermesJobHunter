...
import JobScraper from './capacitor-plugins'
...
export default function JobSearcher({ profile }) {
  ...
  const handleSearchAll = async () => {
    ...
    const hasAndroidScraper = JobScraper && typeof JobScraper.scrapeLinkedIn === 'function'
    ...
    async function scrapePlatform(platformName, email, password) {
      ...
      if (hasAndroidScraper && platformName === 'linkedin') {
        console.log('[JobSearcher] Calling native scrapeLinkedIn...')
        const result = await JobScraper.scrapeLinkedIn({ email, password, keywords, location: 'Bangalore' })
        console.log('[JobSearcher] scrapeLinkedIn result:', JSON.stringify(result))
        return result?.jobs || []
      }
      if (hasAndroidScraper && platformName === 'naukri') {
        console.log('[JobSearcher] Calling native scrapeNaukri...')
        const result = await JobScraper.scrapeNaukri({ email, password, keywords, location: 'Bangalore' })
        console.log('[JobSearcher] scrapeNaukri result:', JSON.stringify(result))
        return result?.jobs || []
      }
      return []
    }
